...
# 5. SpargeAttn (Sparse + Sage Attention - block-sparse quantized attention)
sparge_attn_fn = None
SPARGE_ATTN_AVAILABLE = False
try:
    from spas_sage_attn import spas_sage2_attn_meansim_topk_cuda as _sparge_fn
    sparge_attn_fn = _sparge_fn
    SPARGE_ATTN_AVAILABLE = True
except (ImportError, AttributeError, OSError):
    pass

# SpargeAttn global configuration (topk ratio for block sparsity)
SPARGE_ATTN_CONFIG = {
    'topk': 0.5,
    'smooth_k': True,
}
...
def validate_attention_mode(requested_mode: str, debug=None) -> str:
    ...
    # SpargeAttn (Sparse + Sage Attention)
    if requested_mode == 'spargeattn':
        ...
        if SAGE_ATTN_2_AVAILABLE:
            if debug:
                debug.log(
                    "SpargeAttn not available. Install from https://github.com/thu-ml/SpargeAttn\n"
                    "Falling back to SageAttention 2.",
                    level="WARNING", category="setup", force=True
                )
            return 'sageattn_2'
        error_msg = (
            "Cannot use 'spargeattn' attention mode: SpargeAttn is not installed.\n"
            "\n"
            "SpargeAttn provides block-sparse acceleration on top of SageAttention2.\n"
            "Falling back to PyTorch SDPA (scaled dot-product attention).\n"
            "\n"
            "To fix this issue:\n"
            "  1. Run install.py in this custom node directory\n"
            "  2. OR: pip install git+https://github.com/thu-ml/SpargeAttn.git\n"
            "  3. OR change attention_mode to 'sageattn_2' or 'sdpa'\n"
            "\n"
...
def call_sparge_attn_varlen(q, k, v, cu_seqlens_q, cu_seqlens_k, max_seqlen_q, max_seqlen_k, **kwargs):
    """
    Wrapper for SpargeAttn that converts varlen format to batched format.
    ...
    if not SPARGE_ATTN_AVAILABLE:
        raise ImportError("SpargeAttn is not available")
    ...
    # SpargeAttn requires seq_len >= 128 and head_dim in {64, 128}
    dim = q.shape[2]
    if max_seqlen_q < 128 or dim not in (64, 128):
        if SAGE_ATTN_2_AVAILABLE:
            return call_sage_attn_2_varlen(
                q, k, v, cu_seqlens_q, cu_seqlens_k,
                max_seqlen_q, max_seqlen_k, **kwargs
...
if not os.environ.get("SEEDVR2_OPTIMIZATIONS_LOGGED"):
    ...
    sparge_status = "✅" if SPARGE_ATTN_AVAILABLE else "❌"
    ...
    available = [SPARGE_ATTN_AVAILABLE, SAGE_ATTN_AVAILABLE, FLASH_ATTN_AVAILABLE, TRITON_AVAILABLE]
    num_available = sum(available)

    if num_available == 4:
        print(f"⚡ SeedVR2+SpargeAttn: SpargeAttn {sparge_status} | SageAttention {sage_status} | Flash Attention {flash_status} | Triton {triton_status}")
    elif num_available == 0:
        print(f"⚠️  SeedVR2+SpargeAttn: SpargeAttn {sparge_status} | SageAttention {sage_status} | Flash Attention {flash_status} | Triton {triton_status}")
        print("💡 Run install.py or: pip install sageattention flash-attn triton")
    else:
        icon = "⚡" if num_available >= 2 else "⚠️ "
        print(f"{icon} SeedVR2+SpargeAttn: SpargeAttn {sparge_status} | SageAttention {sage_status} | Flash Attention {flash_status} | Triton {triton_status}")

        # Build install suggestions for missing packages
        missing = []
        if not SPARGE_ATTN_AVAILABLE:
            missing.append("spas_sage_attn (run install.py)")
